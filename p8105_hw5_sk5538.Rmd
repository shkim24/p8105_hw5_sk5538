...
```
As the group size increases, the probability of shared birthday increases. However, the relationship is not perfectly linear. We observe an increased rate of increase as group size exceeds 10, and a decreased rate of increase as group size exceeds 30. The probability approaches nearly 100% as the group size approaches 50. 
...
ggplot(power_results, aes(x = mu, y = power)) +
  ...
  theme_minimal()
```
There is a positive association between power and μ (effect size). This means that having a larger effect size makes it easier to detect a significant effect, which increases the probability of rejecting the null hypothesis. The positive association is not linear. The rate of increase of power is steep from 1 to 3, but noticeably slows down afterwards. When μ exceeds 4, the power remains close to 1.  
...
ggplot(estimate_results, aes(x = mu)) +
  ...
  theme_minimal()
```
Yes. When we reject the null, we are identifying tests where the sample μ^ reflects the true value. This leads the average of these μ^ to approximate the true μ, especially when the we have a large number of tests (law of large numbers), which we do in this case. 
...
```
The data contains `r nrow(homicide_df)` criminal homicides over a decade in 50 largest cities in America. The variables include `r colnames(homicide_df)`. The data was cleaned and standardized to ensure accuracy. The data can provide insights into patterns in urban violence.  
